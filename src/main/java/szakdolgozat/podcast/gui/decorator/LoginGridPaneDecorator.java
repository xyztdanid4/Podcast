...
public class LoginGridPaneDecorator extends Decorator {
	...
	/*
	 * public static TextFieldSample decorateTextFieldFactory(TextFieldSample
	 * textField) { textField.setBorder(new Border(new
	 * BorderStroke(Color.web(BORDERCOLOR), BorderStrokeStyle.SOLID, new
	 * CornerRadii(CORNERRADIUS), new BorderWidths(BORDERSIZE))));
	 * textField.setBackground(new Background( new
	 * BackgroundFill(Color.web(ITEMBACKGROUNDCOLOR), new
	 * CornerRadii(CORNERRADIUS), Insets.EMPTY))); textField.setStyle(
	 * "-fx-text-inner-color: white;"); return textField; }
	 */
}
